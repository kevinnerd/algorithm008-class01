...
class Solution {
    ...
    public char[][] updateBoard(char[][] board, int[] click) {
        ...
        doUpdate(click[0], click[1]);
        return board;
    }

    private void doUpdate(int x, int y) {
        if (isLocationOutOfRange(x, y)) {
            return ;
        }
        ...
        if (get(x, y) == 'E') {
            int count = countMinesNearBy(x, y);
            if (count > 0) {
                change(x, y, (char) (count + '0'));
            } else {
                change(x, y, 'B');
                for (int i = 0; i < 8; i++) {
                    doUpdate(x + dx[i], y + dy[i]);
                }
            }
        }
    }

    private boolean isLocationOutOfRange(int x, int y) {
    ...
    private void change(int x, int y, char target) {
        board[x][y] = target;
    }

    private int countMinesNearBy(int x, int y) {
        int count = 0;
        for (int i = 0; i < 8; i++) {
            int new_x = x + dx[i], new_y = y + dy[i];
            if (!isLocationOutOfRange(new_x, new_y) && 
                get(new_x, new_y) == 'M') {
                count++;
            }
        }
